...
export const PUT = requirePermission('activities.update', async (req: Request, { params }: { params: { id: string } }) => {
  try {
    const data = await req.json();
    const updated = await (prisma.activite as any).update({
      where: { id: params.id },
      data
    });
...
export const DELETE = requirePermission('activities.delete', async (req: Request, { params }: { params: { id: string } }) => {
  try {
    await (prisma.activite as any).delete({
      where: { id: params.id }
    });
