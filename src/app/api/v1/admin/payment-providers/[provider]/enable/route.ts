...
/**
 * POST /api/v1/admin/payment-providers/[provider]/enable
 * Active un provider. Requiert que les credentials soient déjà configurés (sauf MANUAL).
 */
async function handler(request: Request, { params }: { params: { provider: string } }) {
  try {
    ...
    const result = await PaymentProviderAdminService.enableProvider(organizationId, params.provider, actorId);
